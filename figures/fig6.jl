...
for (i, f) ∈ enumerate(files)
    ...
    jldopen(f) do ds
        ...
        axσ[i].title = panel_labels[i] * " Experiment " * all_labels[i]
        ...
        if i ∈ (1, 2)
            ...
            if i % 2 == 0
                axprofile[1].title = "(a) t = $(round(Int, t[profile_time]/60))min"
            end
        ...
        elseif i ∈ (3, 4)
            lines!(axprofile[2], σ_ha′[profile_time, :], _z, color = Makie.wong_colors()[i],
                    linestyle = ls, label = all_labels[i])
            if i % 2 == 0
                axprofile[2].title = "(b) t = $(round(Int, t[profile_time]/60))min"
            end
        ...
        elseif i ∈ (5, 6)
            lines!(axprofile[3], σ_ha′[profile_time, :], _z, color = Makie.wong_colors()[i],
                    linestyle = ls, label = all_labels[i])
            if i % 2 == 0
                axprofile[3].title = "(c) t = $(round(Int, t[profile_time]/60))min"
            end
        end
    end
end
...
colsize!(fig.layout, 1, Relative(1/3.5))
##
save("fig6.png", fig)
