...
titles = ("(a) Experiment II′",
    "(b) Experiment V′")
ax = [Axis(fig[i, 1], ylabel=L"\Delta C / \Delta C_{0}") for i ∈ 1:3]
ts_range = 1:420
for (i, file) ∈ enumerate(files)
    ...
    S_line = lines!(ax[i], t, ΔS ./ ΔS[1], color=:blue, label=L"ΔS / ΔS_{0}")
    T_line = lines!(ax[i], t, ΔT ./ ΔT[1], color=:red, label=L"ΔΘ / ΔΘ_{0}")
    ylims!(ax[i], 0, 1.1)
    ax[i].title = titles[i]
    ax2 = Axis(fig[i, 1],
        yaxisposition=:right,
        ...
        ylabel=L"R_{\rho}")
    Rᵨ_line = lines!(ax2, t, Rᵨ, color=:dodgerblue, label=L"R_{\rho}")
    ylims!(ax2, 1.04, 1.39)
    linkxaxes!(ax2, ax[i])
    ...
    leglabels = Any[]
    for a in [ax2, ax[i]]
        pl, lb = Makie.get_labeled_plots(a, merge=false, unique=false)
        append!(plots, pl)
        append!(leglabels, lb)
    end
    Legend(fig[i, 1], plots, leglabels,
