...
function save_diagnostics!(diagnostics_file::AbstractString, tracers::AbstractString,
    ...
    eos = isnothing(eos) : "" : eos * "/" # creates a group in the saved output.
...
"""
    function compute_Ẽ(diagnostics_file::AbstractString, co::AbstractString)
...
"""
function Ẽ(diagnostics_file::AbstractString, co::AbstractString, eos)
    ...
    save_Ẽ!(diagnostics_file, Ẽ, eos)

    return nothing
end
function save_Ẽ!(diagnostics_file, Ẽ, eos)
    ...
    if isfile(diagnostics_file)
        ...
        jldopen(diagnostics_file, "w") do file
            # copy from above once it is finished
        end
    end
    return nothing
end
