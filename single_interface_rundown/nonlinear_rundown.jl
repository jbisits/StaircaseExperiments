...
architecture = GPU()
diffusivities = (ν=7e-4, κ=(S=7e-7, T=1e-5))
domain_extent = (Lx=0.1, Ly=0.1, Lz=-0.5)
domain_topology = (x = Periodic, y = Periodic, z = Bounded)
resolution = (Nx=60, Ny=60, Nz=250)
...
interface_ics = SingleInterfaceICs(eos, depth_of_interface, salinity, temperature,
                                    interface_smoothing = TanhInterfaceThickness(0.005, 0.01))
initial_noise = NoiseAtDepth([-0.26, -0.24], VelocityNoise(1e-4))
## setup model
